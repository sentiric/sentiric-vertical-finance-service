...
RUN CGO_ENABLED=0 GOOS=linux go build \
    ...
    -o /app/bin/sentiric-vertical-finance-service ./cmd/vertical-finance-service
...
COPY --from=builder /app/bin/sentiric-vertical-finance-service .
RUN chown appuser:appgroup ./sentiric-vertical-finance-service

# GÜVENLİK: Kullanıcıyı değiştir
USER appuser

ENTRYPOINT ["./sentiric-vertical-finance-service"]